...
function Slider({
  ...
  const _values = React.useMemo<number[]>(
    () => (isNumberArray(value) ? value : isNumberArray(defaultValue) ? defaultValue : [min, max]),
      ...
      <SliderPrimitive.Control className="data-vertical:min-h-40 relative flex w-full touch-none items-center select-none data-disabled:opacity-50 data-vertical:h-full data-vertical:w-auto data-vertical:flex-col">
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            ...
            key={index}
            className="border-primary ring-ring/50 size-4 rounded-full border bg-white shadow-sm transition-[color,box-shadow] hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden block shrink-0 cursor-pointer select-none disabled:pointer-events-none disabled:opacity-50"
          />
        ))}
